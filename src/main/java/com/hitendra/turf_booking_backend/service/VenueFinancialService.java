...
public class VenueFinancialService {

    private final BookingRepository bookingRepository;
    private final RefundRepository refundRepository;
    private final ExpenseRepository expenseRepository;
    private final ServiceRepository serviceRepository; // For venue validation

    public VenueFinancialSummary getDashboardSummary(Long venueId, LocalDate startDate, LocalDate endDate) {
        ...
        Double grossRevDouble = bookingRepository.calculateGrossRevenue(venueId, startDate, endDate);
        BigDecimal grossRevenue = grossRevDouble != null ? BigDecimal.valueOf(grossRevDouble) : BigDecimal.ZERO;

        Double cancelledRevDouble = bookingRepository.calculateCancelledRevenue(venueId);
        BigDecimal cancelledRevenue = cancelledRevDouble != null ? BigDecimal.valueOf(cancelledRevDouble) : BigDecimal.ZERO;

        // 2. Refund Metrics
        BigDecimal refundedRevenue = refundRepository.calculateRefundedRevenue(venueId, startDate, endDate);
        ...
        BigDecimal advancePending = bookingRepository.calculateAdvancePending(venueId);
        if (advancePending == null) advancePending = BigDecimal.ZERO;

        BigDecimal advanceReceived = bookingRepository.calculateAdvanceReceived(venueId);
        if (advanceReceived == null) advanceReceived = BigDecimal.ZERO;

        // 5. Cash Collection (Venue side)
        BigDecimal cashCollected = bookingRepository.calculateCashCollected(venueId);
        if (cashCollected == null) cashCollected = BigDecimal.ZERO;

        // 6. Expense Metrics
        BigDecimal totalExpenses = expenseRepository.sumAmountByVenueId(venueId);
        if (totalExpenses == null) totalExpenses = BigDecimal.ZERO;

        BigDecimal expensesCash = expenseRepository.sumAmountByVenueIdAndPaymentMode(venueId, PaymentMode.CASH);
        if (expensesCash == null) expensesCash = BigDecimal.ZERO;

        BigDecimal expensesBank = expenseRepository.sumAmountByVenueIdAndPaymentMode(venueId, PaymentMode.BANK_TRANSFER);
    ...
    public CashBankSummary getCashBankSummary(Long venueId) {
        BigDecimal cashCollected = bookingRepository.calculateCashCollected(venueId);
        if (cashCollected == null) cashCollected = BigDecimal.ZERO;
    ...
    @Transactional
    public ExpenseResponse addExpense(Long venueId, ExpenseRequest request) {
        com.hitendra.turf_booking_backend.entity.Service venue = serviceRepository.findById(venueId)
    ...
    public List<ExpenseResponse> getExpenses(Long venueId, LocalDate startDate, LocalDate endDate) {
        if (startDate == null) startDate = LocalDate.now().withDayOfMonth(1);
        if (endDate == null) endDate = LocalDate.now();

        return expenseRepository.findByVenueIdAndExpenseDateBetweenOrderByExpenseDateDesc(venueId, startDate, endDate)
                .stream()
    ...
    private ExpenseResponse mapToResponse(Expense expense) {
        return ExpenseResponse.builder()
                .id(expense.getId())
                .venueId(expense.getVenue().getId())
